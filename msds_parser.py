...
# ---------------------------------------------------------------------------
# 8. 노출기준 (TWA / ERPG,PAC 등 응급노출기준)
# ---------------------------------------------------------------------------
def parse_exposure_limits(section8_text):
    result = {"twa": "", "erpg_pac": ""}
    m = re.search(r'TWA\s*[:：]?\s*([\d.]+\s*(?:ppm|mg/m3|mg/㎥|㎎/㎥))', section8_text)
    if m:
        result["twa"] = m.group(1).strip()
    m = re.search(r'((?:ERPG|PAC|IDLH)[\w\-]*\s*[:：]?\s*[\d.]+\s*(?:ppm|mg/m3|mg/㎥|㎎/㎥))', section8_text)
    if m:
        result["erpg_pac"] = m.group(1).strip()
    return result
...
def parse_msds(file_bytes, filename=""):
    ...
    sec8 = get_section_text(full_text, "s8")
    ...
    exposure = parse_exposure_limits(sec8)
